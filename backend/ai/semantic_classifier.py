...
from llm_router import route_llm_call, is_any_provider_available, LLMRouterError, PROVIDER_MODEL
...
def classify(masked_text: str, entity_count: int = 0, semantic_chunks: list[dict] | None = None) -> dict:
    ...
    for attempt in range(MAX_PARSE_RETRIES + 1):
        ...
        try:
            ...
            # Underscore-prefixed: not part of ai/schema.json's public contract,
            # routes.py pops these before building the client-facing ECIResult -
            # they exist only to reach the audit trail (backend/audit/).
            parsed["_llm_provider"] = provider_used
            parsed["_llm_model"] = PROVIDER_MODEL.get(provider_used)
            return parsed
